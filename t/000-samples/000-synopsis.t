...
subtest '... meta test' => sub {
    ...
    subtest '... test Point' => sub {
...

        {
            ...
            is_deeply([ map $_->original, $m->get_code_attributes ], ['wo'], '... we show one CODE attribute');
        }

        {
            my $m = $Point->get_method( 'y' );
            is_deeply([ map $_->original, $m->get_code_attributes ], ['ro'], '... we show one CODE attribute');
        }
